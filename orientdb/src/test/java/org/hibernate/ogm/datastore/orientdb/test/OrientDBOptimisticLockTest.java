...
import org.hibernate.ogm.datastore.orientdb.test.jpa.Writer;
...
public class OrientDBOptimisticLockTest {
	...
	public void test1ParallelUpdateEntity() throws Exception {
		...
		CountDownLatch readyForWrite = new CountDownLatch( 2 );
	...
	private class WriterUpdateThread implements Callable<Long> {
		...
		private final CountDownLatch readyForWrite;

		public WriterUpdateThread(long taskId, long writerId, EntityManager localEm, CountDownLatch readyForWrite) {
			this.taskId = taskId;
	...
	private class WriterDeleteThread implements Callable<Long> {
		...
		public WriterDeleteThread(long taskId, long writerId, EntityManager localEm, CountDownLatch readyForWrite) {
			this.taskId = taskId;
			this.writerId = writerId;
			this.localEm = localEm;
			this.readyForWrite = readyForWrite;
